...
namespace MapsetParser.starrating.osu.preprocessing
{
    public class OsuDifficultyHitObject : DifficultyHitObject
    {
        ...
        private const double TAIL_LENIENCY = -36;   // Taken from https://github.com/ppy/osu/blob/1582536f04613a3f659fd2284afc3939caed465c/osu.Game/Rulesets/Objects/SliderEventGenerator.cs#L11
        ...
        public OsuDifficultyHitObject(HitObject hitObject, HitObject lastObject, HitObject? lastLastObject, List<DifficultyHitObject> objects, int index)
            : base(hitObject, lastObject, objects, index)
        {
            ...
            {
                HitWindowGreat = 2 * new HitWindows().WindowFor(HitResult.Great);
            }
            else
            {
                HitWindowGreat = 2 * new HitWindows().WindowFor(HitResult.Great);
        ...
        private void setDistances()
        {
            ...
            {
                double lastTravelTime = Math.Max(lastSlider.LazyTravelTime, min_delta_time);
                ...
                float tailJumpDistance = Vector2.Subtract(lastSlider.UnstackedEndPosition + lastSlider.StackOffset(), BaseObject.StackedPosition()).Length() * scalingFactor;
        ...
        private void computeSliderCursorPosition(Slider slider)
        {
            ...
            double trackingEndTime = Math.Max(
                slider.time + slider.Duration() + TAIL_LENIENCY,
                slider.time + slider.Duration() / 2
            );

            List<HitObject> nestedObjects = slider.NestedHitObjects();

            SliderTick? lastRealTick = slider.NestedHitObjects().OfType<SliderTick>().LastOrDefault();

            if (lastRealTick?.time > trackingEndTime)
            {
            ...
            slider.LazyTravelTime = trackingEndTime - slider.time;
            ...
            slider.LazyEndPosition = slider.StackedPosition() + slider.GetPathPosition(endTimeMin); // temporary lazy end position until a real result can be derived.
